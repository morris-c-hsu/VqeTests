...
with matplotlib backend and iqp style for professional-quality diagrams.
"""
...
def save_circuit_diagram(circuit, filename, description, output_dir='../docs/images'):
    """Save circuit diagram as PNG using Qiskit's native drawer with iqp style."""
    ...
    try:
        # Use Qiskit's built-in circuit drawer with matplotlib backend and iqp style
...
def main():
    ...
    for i, (circuit, filename, description) in enumerate(circuits, 1):
        print(f"\n{i}. {description}")
        print("-" * 70)

        if save_circuit_diagram(circuit, filename, description):
            success_count += 1
        total_count += 1
